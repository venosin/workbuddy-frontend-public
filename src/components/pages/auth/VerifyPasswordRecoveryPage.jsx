...
export function VerifyPasswordRecoveryPage() {
  ...
  useEffect(() => {
    // Obtener el mensaje del state al cargar la página
    if (location.state && location.state.message) {
      setMessage(location.state.message);
    } else {
      // Si no hay datos en el state, redirigir a la página de solicitud
      navigate('/recuperar-contrasena');
    }
  }, [location, navigate]);

  const handleChange = (e) => {
    // Eliminar espacios y limitar a 6 caracteres
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await authService.verifyPasswordRecoveryCode(verificationCode);
      ...
      setTimeout(() => {
        navigate('/restablecer-contrasena', { 
          state: { 
            verified: true,
            message: 'Código verificado. Ahora puedes establecer una nueva contraseña.'
          } 
        });
      }, 2000);
    } catch (error) {
