...
export function OrderCheckoutPage() {
  ...
  const updateFormWithAddress = useCallback(
    (address) => {
      if (!address) return;
  ...
  const loadUserAddresses = useCallback(async () => {
    ...
    try {
      ...
      if (response && response.addresses && response.addresses.length > 0) {
        ...
        console.log(
          "¿Se encontró dirección por defecto?",
          defaultAddress ? "Sí" : "No"
        );
        ...
        console.log(
          "No se encontraron direcciones en la respuesta directa, intentando obtener configuración completa..."
        );
        ...
        if (settings && settings.addresses && settings.addresses.length > 0) {
          console.log(
            "Direcciones encontradas en configuración completa:",
            settings.addresses
          );
          ...
          setOrderError(
            "No se encontraron direcciones guardadas. Por favor, agrega una nueva dirección."
          );
        }
      }
    } catch (error) {
      ...
      setOrderError(
        "Error al cargar tus direcciones guardadas. Puedes agregar una nueva dirección."
      );
    } finally {
  ...
  const handleSaveNewAddress = async () => {
    ...
    try {
      ...
      if (response.success && response.address) {
        ...
        console.log(
          "La dirección se guardó pero no tenemos los detalles, recargando direcciones..."
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Preparar la dirección de envío en el formato estructurado que espera el backend
      let shippingAddress;

      // Si tenemos una dirección seleccionada de las guardadas, usamos esa
      ...
      // Si no tenemos una dirección estructurada, intentamos parsear el texto
      if (!shippingAddress && formData.shippingAddress) {
        // Intentar dividir la dirección de texto en sus componentes
        const addressParts = formData.shippingAddress
          .split(",")
          .map((part) => part.trim());
        ...
        if (addressParts.length >= 3) {
          shippingAddress = {
            ...
            postalCode: addressParts[3] || "12345", // Código postal por defecto si no existe
          };
        } else {
          // Valores por defecto si no podemos parsear correctamente
          shippingAddress = {
      ...
      const orderData = {
        CartId: cartId, // Asegúrate de que esto esté definido
        payMethod: formData.payMethod,
        shippingAddress: {
          // Nota la ortografía: Address con doble 'd'
      ...
      setOrderError(
        "Ocurrió un error al procesar tu pedido. Por favor intenta más tarde."
      );
    } finally {
          ...
          <div className="bg-white rounded-lg shadow-sm p-8 max-w-md w-full">
            {isSubmitting && !orderSuccess && (
              <div className="text-center">
                ...
                <p className="mt-2 text-gray-600">
                  Estamos procesando tu orden, por favor espera un momento...
                </p>
              </div>
            )}
            ...
            {orderSuccess && (
              <div className="text-center">
                ...
                <p className="mt-2 text-gray-600">
                  Tu orden ha sido creada. Redirigiendo a la página de
                  confirmación...
                </p>
                  ...
                  {showNewAddressForm ? (
                    <div className="border rounded-lg p-4 bg-gray-50">
                      ...
                      <button
                        ...
                        } text-white py-2 px-4 rounded-md transition duration-200 ease-in-out flex justify-center items-center`}
                      >
                        {savingAddress ? (
                          <>
                            <svg
